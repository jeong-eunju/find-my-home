...
- 내가 원하는 옵션을 선택했을 때, **예산 안에서 이사할 수 있는 집**은 어디가 있을까?
:::
...
우리는 예산 내에서 **어떤 조건이 집값을 높이는 데 크게 기여하는지** 파악하고자  
수업시간에 배운 `LassoCV` 모델을 활용하여 분석을 진행했습니다.

::: callout-note
### 📌 Lasso 회귀란?

- 중요도가 낮은 변수의 계수를 자동으로 **0으로 만들고**
- 중요한 변수는 **살아남게** 하는 회귀 기법입니다.
- 해석이 쉽고, **과적합 방지**에 효과적입니다.
:::

::: callout-note
### 🧠 LassoCV 회귀란?

- `LassoCV`는 Lasso 회귀에서 **최적의 정규화 강도(alpha)** 를 **교차검증으로 자동 선택**합니다.
- 여러 alpha 후보 중 가장 성능이 좋은 것을 **데이터 기반으로 선택**합니다.
- 결과적으로 **예측 성능 + 해석력** 모두 확보할 수 있습니다.
:::
...
plt.tight_layout()
plt.savefig("images/lasso_top20.png")
...
plt.tight_layout()
plt.savefig("images/lasso_group_mean.png")
...
display(pd.DataFrame([best_condition]))
print(f"✅ 모델이 예측한 최고의 집값: ${best_price:,.0f}")
```
...
| **분석 내용** | **설명** |
|--------------|--------------------------------|
| **Lasso 요약**             | `LassoCV` 모델은 교차검증으로 최적의 `alpha ≈ 0.5`를 선택하고, 중요하지 않은 변수는 자동으로 제거함으로써 과적합을 방지하고 해석력을 높임. |
